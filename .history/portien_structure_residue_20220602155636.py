...
def read_pdb(pdb_file):
    ...
    for line in pdb_file:
        ...
        if line[0:6] == 'SEQRES':
            ...
            if chain_id in seq:
                ...
                seq[chain_id] = seq[chain_id] + line_seq
            else:
    ...
    for line in pdb_file:
        ...
        if line[0:4] == 'ATOM' and line[13:16] == 'CA ':
            ...
            amino_acid_res_name = line[17:20]
            for chain_id in seq:
                for aa_name in seq[chain_id]:
                    if amino_acid_res_name in seq[chain_id]:
                        index = seq[chain_id].index(amino_acid_res_name)
                        seq[chain_id][index] = [seq[chain_id][index]] + [
                            float(line[30:38]),
                            float(line[38:46]),
                            float(line[46:54])
                        ]
...
def get_protein_coordinates_in_seq(protein_name, protein_sequence, chain_id):
    protien_coords = []
    for protien_name in protein_sequence[chain_id]:
        if (protien_name[0] == protein_name):
            protien_coords.append(protien_name[1:])
...
def main():
    ...
    print(get_protein_coordinates_in_seq('VAL', seq, 'C'))
