...
def read_pdb(pdb_file):
    ...
    for line in pdb_file:
        ...
        if line[0:6] == 'SEQRES':
            ...
            # check if the chain id is in the dictionary of sequences
            if chain_id in seq:
                # if it is, check if the amino acid id is in the dictionary of coordinates
                if aa_id in coords:
                    # if it is, append the coordinates to the list
                    coords[chain_id].append([x, y, z])
                else:
                    # if it is not, add the amino acid id and the coordinates to the dictionary
                    coords[chain_id].append([x, y, z])
                    coords[chain_id].append([x, y, z])
                    # update the index of the amino acid sequence
                    index += 1
    # close the pdb file
    pdb_file.close()
