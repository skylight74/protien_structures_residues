...
def read_pdb(pdb_file):
    ...
    for line in pdb_file:
        ...
        index = 0
        # read the ATOM record and check if the atom name is CA
        elif line[0:4] == 'ATOM':
            if line[12:16] == 'CA  ':
                ...
                # add the amino acid id to the amino acid sequence
                seq[chain_id][index] = aa_id
                # increase the index by one
                index += 1
                # if the index is greater than 20, break the loop
                if index > 20:
                    break
                # if the index is less than 20, continue the loop
                else:
                    continue
                # close the pdb file
