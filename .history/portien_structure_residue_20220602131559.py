...
# the following fuction reads the PDB file amino acid sequences only as the SEQRES records and the coordinates of each amino acid as the ATOM records and return the amino acid sequences and the coordinates as a dictionary
def read_pdb(pdb_file):
    ...
    # initialize the amino acid sequences and the coordinates
    seq = []
    coords = []

    # read the pdb file and get the amino acid sequences and the coordinates
    for line in lines:
        if line[0:4] == 'ATOM':
            if line[13:15] == 'CA':
