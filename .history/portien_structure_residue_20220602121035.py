...
# this is the main function that reads the PDB file, computes the pairwise distances, and outputs the results to the user.
def main():
    if len(sys.argv) != 4:
        print("Usage: python3 pdb_proximity.py <PDB file> <S> <D>")
        sys.exit(1)
    pdb_file = sys.argv[1]
    S = int(sys.argv[2])
    D = float(sys.argv[3])
    ...
    for chain_id in atom_coordinates:
        chain_sequence = read_seqres(atom_coordinates[chain_id][0])
        ...
        print("Close pairs: " + str(close_pairs))
        print()
