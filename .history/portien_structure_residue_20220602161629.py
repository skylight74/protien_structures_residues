...
def get_pairwise_distance(protein_sequence, S, D):
    ...
    for chain_id in protein_sequence:
        ...
        for i in protein_sequence[chain_id]:
            # get the protein coordinates in the chain
            protein_coordinates = get_protein_coordinates_in_seq(i[0], protein_sequence, chain_id)
            # get the pairwise distance between all amino acids in the chain
            for j in protein_coordinates:
                for k in protein_coordinates:
                    # print(distance(j, k))
                    if distance(j, k) <= D and distance(j, k) >= S:
                        # print(j, k)     
# run the program
def main():
