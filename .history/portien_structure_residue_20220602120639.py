...
# this function compute the pairwise distances using the sequence and coordinates and will only return the distances that are less than or equal to D and if the distances are greater than or equal to S
def compute_pairwise_distances(sequence, coordinates, D, S):
    # Initialize the pairwise distances.
    pairwise_distances = []
    # For each amino acid in the sequence,
    for i in range(len(sequence)):
        # Get the amino acid.
        amino_acid_i = sequence[i]
        # Get the amino acid coordinates.
        coordinates_i = coordinates[i]
        # For each amino acid in the sequence,
        for j in range(i + 1, len(sequence)):
            # Get the amino acid.
            amino_acid_j = sequence[j]
            # Get the amino acid coordinates.
            coordinates_j = coordinates[j]
            # If the amino acids are in the same chain,
            if amino_acid_i[0] == amino_acid_j[0]:
                # Compute the euclidean distance between the amino acids.
                distance = euclidean_distance(coordinates_i, coordinates_j)
                # If the distance is less than or equal to D,
                if distance <= D:
                    # Append the amino acids to the pairwise distances.
                    pairwise_distances.append([amino_acid_i, amino_acid_j, distance])
