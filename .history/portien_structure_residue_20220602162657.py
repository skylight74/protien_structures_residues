...
def get_pairwise_distance(protein_sequence, S, D):
    ...
    for chain_id in protein_sequence:
        ...
        for i in range(len(protein_coordinates)):
            for j in range(i + S, len(protein_coordinates)):
                pairwise_distance[chain_id][i, j] = distance(
                    protein_coordinates[i], protein_coordinates[j])
