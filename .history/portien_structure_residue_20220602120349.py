...
# this function compute the pairwise distances using the sequence and coordinates
def compute_pairwise_distances(sequence, coordinates):
    # Initialize the pairwise distances.
    pairwise_distances = []
    # For each chain,
    for chain in sequence:
        # Initialize the chain_pairwise_distances.
        chain_pairwise_distances = []
        # Get the chain ID.
        chain_id = chain[0]
        # Get the sequence.
        chain_sequence = chain[1]
        # For each amino acid,
        for i in range(len(chain_sequence)):
            # Get the amino acid.
            amino_acid = chain_sequence[i]
            # Get the coordinates of the amino acid.
            coordinates1 = coordinates[i]
            # For each amino acid,
            for j in range(i + 1, len(chain_sequence)):
                # Get the amino acid.
                amino_acid2 = chain_sequence[j]
                # Get the coordinates of the amino acid.
                coordinates2 = coordinates[j]
                # Compute the euclidean distance.
                distance = euclidean_distance(coordinates1, coordinates2)
                # Append the amino acid pair and distance to the chain_pairwise_distances.
                chain_pairwise_distances.append([amino_acid, amino_acid2, distance])
        # Append the chain_pairwise_distances to the pairwise distances.
        pairwise_distances.append([chain_id, chain_pairwise_distances])
    # Return the pairwise distances.
    return pairwise_distances

# this function check if the distance is less than or equal to D and if the distance is greater than or equal to S
def check_distance(distance, D, S):
    # If the distance is less than or equal to D,
    if distance <= D:
        # Return True.
        return True
    # If the distance is greater than or equal to S,
    elif distance >= S:
        # Return False.
        return False
    # Otherwise,
