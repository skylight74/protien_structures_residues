...
def compute_pairwise_distances(coordinates):
    ...
    for chain_ID in coordinates:
        ...
        for residue_number in coordinates[chain_ID]:
            ...
            current_coordinates = coordinates[chain_ID][residue_number]['coordinates']
            ...
            for other_residue_number in coordinates[chain_ID]:
                ...
                other_coordinates = coordinates[chain_ID][other_residue_number]['coordinates']
                # Compute the Euclidean distance between the two amino acids
                distance = euclidean_distance(current_coordinates, other_coordinates)
                # Add the distance to the dictionary
                pairwise_distances[chain_ID][residue_number][other_residue_number] = distance
    # Return the pairwise distances
    return pairwise_distances

# the following function will report the amino acid pairs that are within the specified distance of each other
def report_pairs(pairwise_distances, chain_ID, residue_number, distance, sequence):
    # Initialize the list to store the amino acid pairs in
    pairs = []
    # loop over all sequence positions
    for i in range(len(sequence)):
        # Loop over the residue numbers
        for other_residue_number in pairwise_distances[chain_ID]:
            # Get the distance between the two amino acids
            other_distance = pairwise_distances[chain_ID][residue_number][other_residue_number]
            # Check if the distance is less than or equal to the specified distance
            if other_distance <= distance:
                # Check if the amino acids are at least S amino acids apart in the sequence
                if other_residue_number[1] - residue_number[1] >= distance:
                    # Add the amino acid pair to the list
                    pairs.append((residue_number, other_residue_number))
        # Return the list of amino acid pairs
    return pairs