...
def read_pdb(pdb_file):
    ...
    for line in pdb_file:
        ...
        if line[0:6] == 'SEQRES':
            ...
            # get the chain id
            aa_id = line[21]
            # check if the amino acid atom is CA
            if line[12:16] == 'CA  ':
                # if it is, get the x, y, and z coordinates
                x = float(line[30:38])
                y = float(line[38:46])
                z = float(line[46:54])
                # add the coordinates to the list of coordinates
                coords[aa_id].append([x, y, z])
                # check if the amino acid id is in the dictionary of coordinates
                if aa_id in coords:
                    # if it is, add the coordinates to the list of coordinates
                    coords[aa_id].append([x, y, z])
                else:
                    # if it is not, initialize the list of coordinates and add the coordinates to the list of coordinates
                    coords[aa_id] = [[x, y, z]]
                    # increment the index of the amino acid sequence
                    index += 1
