...
def read_pdb(pdb_file):
    ...
    for line in pdb_file:
        ...
        if line[0:4] == 'ATOM' and line[13:16] == 'CA ':
            ...
            if amino_acid_res_name in coords:
                #append the coordinate to the list
                coords[amino_acid_res_name].append([float(line[30:38]), float(line[38:46]), float(line[46:54])])
            else:
                #if it is not, initialize the list and add the coordinate to the list
                coords[amino_acid_res_name] = [[float(line[30:38]), float(line[38:46]), float(line[46:54])]]
    # close the pdb file
    pdb_file.close()
