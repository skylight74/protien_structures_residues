...
def get_distance_between_all_amino_acids(protein_coordinates, chain_id, D,S):
    ...
    for i in range(len(protein_coordinates[chain_id])):
        for j in range(i+1, len(protein_coordinates[chain_id])):
            if distance(protein_coordinates[chain_id][i][1], protein_coordinates[chain_id][j][1]) <= D and abs(i-j) >= S:
                distance_list.append([protein_coordinates[chain_id][i][0], protein_coordinates[chain_id][j][0]])
                distance_list.append([distance(protein_coordinates[chain_id][i][1], protein_coordinates[chain_id][j][1]),(abs(i-j))])
...
def main():
    ...
    # print(seq)
    print(get_distance_between_all_amino_acids(seq, 'A',D,S))
