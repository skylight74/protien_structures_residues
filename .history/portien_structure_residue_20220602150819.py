...
def read_pdb(pdb_file):
    ...
    for line in pdb_file:
        ...
        if line[0:4] == 'ATOM' and line[13:16] == 'CA ':
            ...
            # create a dictionary with the amino acid name as the key and the coordinates as the value
            coords[amino_acid_res_name] = [float(line[30:38]), float(line[38:46]), float(line[46:54])]
            for chain_id in seq:
                [amino_acid_res_name] = [float(line[30:38]), float(line[38:46]), float(line[46:54])]
    # close the pdb file
    pdb_file.close()
