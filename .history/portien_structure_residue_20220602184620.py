...
def get_distance_between_all_amino_acids(protein_coordinates, chain_id, D):

    distance_list = []
    for i in range(len(protein_coordinates[chain_id])):
        for j in range(i+1, len(protein_coordinates[chain_id])):
            if distance(protein_coordinates[chain_id][i][1], protein_coordinates[chain_id][j][1]) <= D:
                distance_list.append([protein_coordinates[chain_id][i][0], protein_coordinates[chain_id][j][0]])
                distance_list.append(distance(protein_coordinates[chain_id][i][1], protein_coordinates[chain_id][j][1]))
    return distance_list
# the following fuction takes a chain id and a list of amino acid coordinates as input and returns the index difference between all amino acids in the chain
def get_index_difference_between_all_amino_acids(protein_coordinates, chain_id,S):
    index_difference_list = []
    for i in range(len(protein_coordinates[chain_id])):
        for j in range(i+1, len(protein_coordinates[chain_id])):
            if abs(i-j) >= S:
                index_difference_list.append([protein_coordinates[chain_id][i][0], protein_coordinates[chain_id][j][0]])
                index_difference_list.append(abs(i-j))
...
def main():
    ...
    # print(seq)
    print(get_distance_between_all_amino_acids(seq, 'A',D))
    print(get_index_difference_between_all_amino_acids(seq, 'A',S))
