...
def read_pdb(pdb_file):
    ...
    for line in pdb_file:
        ...
        index = 0
        coords[index] = []
                # initialize the index of the amino acid sequence
                # read the ATOM record
        if line[0:4] == 'ATOM':
            # check if the atom is an alpha carbon
            if line[12:16] == 'CA  ':
                # get the amino acid serial number
                serial = int(line[6:11])
                # append the coordinates to the list of coordinates
                coords[index].append([float(line[30:38]), float(line[38:46]), float(line[46:54])])
                # check if the amino acid serial number is in the dictionary of coordinates
                if serial in coords:
                    # if it is, append the coordinates to the list
                    coords[serial].append([float(line[30:38]), float(line[38:46]), float(line[46:54])])
                else:
                    # if it is not, initialize the list and add the coordinates to the list
                    coords[serial] = [float(line[30:38]), float(line[38:46]), float(line[46:54])]
                    # increase the index of the amino acid sequence
                    index += 1
                    # initialize the list of coordinates
                    coords[index] = []
