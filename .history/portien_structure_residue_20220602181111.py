...
# the follwoing fuction computes how far the amino acid is from another amino acid in the sequence
def distance_in_sequence(protein_sequence, chain_id, amino_acid_id1, amino_acid_id2):
    print(protein_sequence[chain_id].index(amino_acid_id1))
    return abs(protein_sequence[chain_id].index(amino_acid_id1) - protein_sequence[chain_id].index(amino_acid_id2))
...
def get_pairwise_distance(protein_sequence, S, D):
    ...
    pairwise_distance = {}
    # initialize the pairwise distance as a dictionary with chain id as the key and amino acid pairwise distance as the value
    for chain_id in protein_sequence:
        # get the protein coordinates in the chain
        protein_coordinates = get_protein_coordinates_in_seq(
            protein_sequence, chain_id)
        # print (protein_coordinates)
        # initialize the pairwise distance as a dictionary with amino acid id as the key and amino acid pairwise distance as the value
        pairwise_distance[chain_id] = {}
        # initialize the pairwise distance as a dictionary with amino acid id as the key and amino acid pairwise distance as the value
        # calculate the distance between each amino acid in the chain the distance is less than or equal to D and which are at least S amino acids apart in the sequence
        for i in range(len(protein_coordinates)):
            for j in range(i + 1, len(protein_coordinates)):
                if distance(protein_coordinates[i], protein_coordinates[j]) <= D and distance(protein_coordinates[i], protein_coordinates[j]) >= S:
                    pairwise_distance[chain_id][i] = protein_coordinates[j]
                    pairwise_distance[chain_id][j] = protein_coordinates[i]
...
def main():
    ...
    print(get_pairwise_distance(seq, S, D))
