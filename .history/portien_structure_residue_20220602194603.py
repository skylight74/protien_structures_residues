...
def get_distance_between_all_amino_acids(protein_coordinates, chain_id, D, S):
    ...
    for i in range(len(protein_coordinates[chain_id])):
        for j in range(i + 1, len(protein_coordinates[chain_id])):
            eucladian_distance = distance(protein_coordinates[chain_id][i][1],
                                          protein_coordinates[chain_id][j][1])
            abs_distance = abs(i - j)
            if eucladian_distance <= D and abs_distance >= S:
                distance_list.append([
                    protein_coordinates[chain_id][i][0], i,
                    protein_coordinates[chain_id][j][0], j, eucladian_distance,
                    abs_distance
                ])
...
def main():
    ...
    match_list = {}
    for chain_id in seq:
        match_list[chain_id] = get_distance_between_all_amino_acids(
            seq, chain_id, D, S)
    # write the output to the output file
    # tabluate the match_list using the chain id as the key and the amino acid pairwise distance as the value
    with open("e2044725_table.out", 'w') as f:
        for chain_id in match_list:
            f.writelines("Chain ID: " + chain_id + "\n")
            f.writelines(
                tabulate(match_list[chain_id],
                         headers=[
                             'Amino Acid 1', 'Index from seq start',
                             'Amino Acid 2', 'Index from seq start',
                             'Euclidian Distance', 'Absolute Distance'
                         ],
                         tablefmt='grid') + "\n")
...
if __name__ == '__main__':
    main()
print("File has been created with the name e2044725_table.out")
