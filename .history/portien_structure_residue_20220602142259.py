...
def read_pdb(pdb_file):
    ...
    for line in pdb_file:
        ...
        if line[0:4] == 'ATOM' and line[13:16] == 'CA ':
            # append the location to the sequence dictionary
            # add the coordinates to the list of coordinates
            index += 1
            # add the coordinates to the anmino acid for all sequences in the sequence dictionary
            for chain_id in seq:
                seq[chain_id][seq[chain_id]].extend([float(line[30:38]), float(line[38:46]), float(line[46:54])])
    # close the pdb file
    pdb_file.close()
