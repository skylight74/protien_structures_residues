    ...
    with open(pdb_file, 'r') as file:
        ...
        for line in file:
            ...
            if line[0:4] == 'ATOM':
                ...
                amino_acid_number = int(line[22:26])
                ...
                z = float(line[46:54])
                # If the chain ID is the first chain ID,
                if chain_id == 'A':
                    # If the amino acid name is not already in the sequence,
                    if amino_acid_name not in sequence:
                        # Add the amino acid name to the sequence.
                        sequence.append(amino_acid_name)
                    # If the amino acid number is not already in the coordinates,
                    if amino_acid_number not in coordinates:
                        # Add the amino acid number and the coordinates to the coordinates.
                        coordinates.append([amino_acid_number, x, y, z])
                # If the chain ID is the second chain ID,
                elif chain_id == 'B':
                    # If the amino acid name is not already in the sequence,
                    if amino_acid_name not in sequence:
                        # Add the amino acid name to the sequence.
            ...
            # If the line starts with SEQRES,
                ...
                amino_acid_sequence = line[19:].split()
                # If the chain ID is the first chain ID,
                if chain_id == 'A':
                    # For each amino acid in the amino acid sequence,
                    for amino_acid in amino_acid_sequence:
                        # If the amino acid is not already in the sequence,
                        if amino_acid not in sequence:
...
# This function computes the pairwise distances between all amino acids in a chain only using the coordinates of the alpha carbons.
def compute_pairwise_distances(coordinates):
    # Initialize the pairwise distances.
    pairwise_distances = []
    # For each amino acid in the coordinates,
    for amino_acid_1 in coordinates:
        # Initialize the distances.
        distances = []
        # For each amino acid in the coordinates,
        for amino_acid_2 in coordinates:
            # Compute the distance between the two amino acids.
            distance = euclidean_distance(amino_acid_1[1:], amino_acid_2[1:])
            # Add the distance to the distances.
            distances.append(distance)
        # Add the distances to the pairwise distances.
