    ...
    for i in range(len(protein_coordinates[chain_id])):
        for j in range(i+1, len(protein_coordinates[chain_id])):
            ...
            if eucladian_distance <= D and abs_distance >= S:
                distance_list.append([protein_coordinates[chain_id][i][0],i, protein_coordinates[chain_id][j][0],j, eucladian_distance, abs_distance])
...
def main():
    ...
    with open("table.out", 'w') as f:
        for chain_id in match_list:
            ...
            f.writelines(tabulate(match_list[chain_id], headers=['Amino Acid 1', 'Index from seq start', 'Amino Acid 2', 'Amino Acid 2 Index', 'Euclidian Distance', 'Absolute Distance'], tablefmt='grid')+"\n")
