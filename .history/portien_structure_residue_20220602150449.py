...
def read_pdb(pdb_file):
    ...
    for line in pdb_file:
        ...
        if line[0:4] == 'ATOM' and line[13:16] == 'CA ':
            ...
            # get amino acid index from the sequence dictionary
            amino_acid_index = seq[chain_id].index(amino_acid_res_name)
            seq[chain_id][amino_acid_index] = {'index': amino_acid_index, 'res_name': amino_acid_res_name}
            x = float(line[30:38])
            y = float(line[38:46])
            z = float(line[46:54])
            seq[chain_id][amino_acid_index]['coord'] = [x, y, z]
    # close the pdb file
    pdb_file.close()
